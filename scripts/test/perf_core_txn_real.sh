...
PG_CONTAINER="${COIN_PERF_PG_CONTAINER:-${COIN_TEST_PG_CONTAINER:-coin-test-postgres}}"
REDIS_CONTAINER="${COIN_PERF_REDIS_CONTAINER:-coin-perf-redis}"

PG_PORT="${COIN_PERF_PG_PORT:-${COIN_TEST_PG_PORT:-55432}}"
PG_USER="${COIN_PERF_PG_USER:-${COIN_TEST_PG_USER:-postgres}}"
PG_PASSWORD="${COIN_PERF_PG_PASSWORD:-${COIN_TEST_PG_PASSWORD:-postgres}}"
PG_DB="${COIN_PERF_PG_DB:-${COIN_TEST_PG_DB:-coin_test}}"
PG_IMAGE="${COIN_PERF_PG_IMAGE:-${COIN_TEST_PG_IMAGE:-postgres:16-alpine}}"
...
export PERF_TXN_RECOVERY_INTERVAL_MS="${PERF_TXN_RECOVERY_INTERVAL_MS:-50}"
export PERF_TXN_RECOVERY_STALE_MS="${PERF_TXN_RECOVERY_STALE_MS:-200}"
export PERF_TXN_RECOVERY_BATCH="${PERF_TXN_RECOVERY_BATCH:-2000}"
...
printf '[perf-core-txn-real] go=%s duration=%ss concurrency=%s warmup=%s timeout_ms=%s webhook_poll_ms=%s webhook_wait_timeout_ms=%s txn_recovery_interval_ms=%s txn_recovery_stale_ms=%s txn_recovery_batch=%s\n' \
  "$GO_BIN" "$PERF_DURATION_SECONDS" "$PERF_CONCURRENCY" "$PERF_WARMUP" "$PERF_REQUEST_TIMEOUT_MS" "$PERF_WEBHOOK_POLL_INTERVAL_MS" "$PERF_WEBHOOK_WAIT_TIMEOUT_MS" "$PERF_TXN_RECOVERY_INTERVAL_MS" "$PERF_TXN_RECOVERY_STALE_MS" "$PERF_TXN_RECOVERY_BATCH"
